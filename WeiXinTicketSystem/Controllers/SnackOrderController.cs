...
namespace WeiXinTicketSystem.Controllers
{
    public class SnackOrderController : RootExraController
    {
        ...
        public async Task<FileResult> ExportExcelDetail(int? OrderStatus, string OrderDateRange, string Search)
        {
            ...
            string[] propertyName = new string[] { "订单明细ID", "订单ID", "套餐编码", "套餐名称", "标准价格", "销售价格", "数量", "合计", "实际支付金额", "创建时间", "修改时间", "优惠券编码", "优惠券名称", "优惠金额", "影院编码", "订单号", "手机号", "套餐数量", "总金额", "释放时间", "提交时间", "凭证号", "订单状态", "退订时间", "取货时间", "送货地址","送货时间","订单支付标志","订单支付类型","订单支付时间","支付交易号","是否使用优惠券","影院名称" };
            //将参数写入到一个临时集合中
